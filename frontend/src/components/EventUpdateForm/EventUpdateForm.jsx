...
const EventUpdateForm = ({ eventInformation }) => {
  ...
  const handleUpdate = async event => {
    ...
    try {
      const response = await axios
        .put(
          `http://localhost:8888/api/events/modify/${eventInformation._id}`,
          formData,
          {
            headers: {
      ...
      console.log(
        'There is an error when try to send PUT REQUEST to http://localhost:8888/api/events/modify/:id',
      );
  ...
  const handleFinishEventClick = (e, eventFinish) => {
    ...
    axios
      .post(
        `http://localhost:8888/api/events/finish/${eventFinish._id}`,
        null,
        {
          headers: {
            Authorization: `Bearer ${exampleTokenForPhuoc}`,
