  ...
  const handleSubmit = async event => {
    ...
    try {
      const response = await axios
        .post('http://localhost:8888/api/events/create', formData, {
          headers: {
      ...
      if (ggToken !== null) {
        let temp = await axios.post(
          'http://localhost:8888/api/users/googleevent',
          {
            EventForm: {
      ...
      console.log(
        'There is an error when try to send POST REQUEST to http://localhost:8888/api/events/create',
      );
